...
# Allow overriding the salt version. Version must be specified in <major>.<minor> format, i.e 2016.11, 2018.3, etc.
SALT_MAJOR_VERSION=${SALT_VERSION:-latest}

cat >> /etc/apt/sources.list.d/saltstack.list << EOF
deb http://repo.saltstack.com/apt/ubuntu/${OS_VERSION}/amd64/${SALT_VERSION} ${OS_CODENAME} main
EOF
